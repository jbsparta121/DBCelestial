...
echo "$($PSQL "SELECT name FROM teams FULL JOIN games ON team_id = games.winner_id WHERE year = 2018 AND round = 'Final'")"
...
echo "$($PSQL "SELECT year,name FROM teams FULL JOIN games ON team_id = games.winner_id WHERE round = 'Final' ORDER BY year")"
